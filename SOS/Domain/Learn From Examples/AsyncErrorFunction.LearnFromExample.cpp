...
namespace domain
{
	namespace learn_from_examples
	{
		...
		void AsyncErrorFunction::individual_selection_error_function_thread_pool(std::function<double(int, unsigned long, unsigned long, unsigned int)> error_function)
		{
			double error;
			struct WorkOrder work_order;
			...
			while (true)
			{
				...
				std::cout << ".";
				error = error_function(work_order.individual_index, work_order.input_start, work_order.input_end, work_order.test_case);
...
//					pushGP::globals::population_agents[work_order.individual_index].set_error_for_all_training_data(error);
