#include "StockForecastingErrorFunction.h"
...
namespace domain
{
	order_types StockForecastingErrorFunction::run_individual_program(static unsigned int individual_index, static unsigned long row)
	{
	...
	void StockForecastingErrorFunction::eval_one_day_of_test_case(static std::vector<int> & _individual_indexes, static unsigned long _row, unsigned int _test_case, bool _record_transactions)
	{
	...
	double StockForecastingErrorFunction::evaluate_individuals(static std::vector<int> & _individual_indexes, static unsigned long _input_start, static unsigned long _input_end, unsigned int _test_case, bool _record_transactions)
	{
...
//		return (error == 0.0 ? std::numeric_limits<double>::max() : error);
	...
	// epsilon-lexicase
	double StockForecastingErrorFunction::lexicase_reproduction_selection_error_function(static int individual_index, static unsigned long input_start_, static unsigned long input_end_)
	{
	...
	double StockForecastingErrorFunction::run_individuals_program(static int individual_index, static unsigned long input_start_, static unsigned long input_end_)
	{
