...
//		Code ins = instruction_list[rng.random(instruction_list.size())];  Debugging this line
	...
	Code parse1(std::istream & is)
	{
		...
		while (is)
		{
			...
			{
//				Code value = Code(boolLiteralFactory.createLiteral(atom == "TRUE" ? true : false)); //new Literal<bool>(atom == "TRUE" ? true : false));
			...
			{
//				Code v = Code(intLiteralFactory.createLiteral(val)); // new Literal<int>(val));
			...
			{
//				Code v = Code(floatLiteralFactory.createLiteral(dbl)); // new Literal<double>(dbl));
