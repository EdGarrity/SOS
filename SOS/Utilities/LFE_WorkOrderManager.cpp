...
namespace Utilities
{
		...
		try
		{
			while (true)
			{
				...
				try
				{
					...
					Utilities::logline_threadsafe << " Finished Order " << std::get<0>(results) << " Score " << std::get<1>(results) /*<< Utilities::endl */;
	...
	void LFE_WorkOrderManager::wait_for_all_threads_to_complete()
	{
		...
		if (num_threads_ > 0)
		{
...
#endif
				debug_message = "wait_for_queue_to_empty,queue_size=" + std::to_string(queue_size);
				Utilities::logline_threadsafe << debug_message /*<< Utilities::endl */;
			...
			Utilities::logline_threadsafe << "Queue empty" /*<< Utilities::endl */ /*<< Utilities::endl */;

			do
			{
				Utilities::logline_threadsafe /*<< Utilities::endl */;
				Utilities::logline_threadsafe /*<< Utilities::endl */;
				Utilities::logline_threadsafe << "N" << std::setw(30) << "effort" << std::setw(30) << "exec_size" << std::setw(30) << "instruction" << std::setw(30) << "individual" << std::setw(30) << "case"  /*<< Utilities::endl */;
				Utilities::logline_threadsafe << "-" << std::setw(30) << "------" << std::setw(30) << "---------" << std::setw(30) << "-----------" << std::setw(30) << "----------" << std::setw(30) << "----" /*<< Utilities::endl */;
				...
				for (long i = 0; i < (long)num_threads_; i++)
				{
					if (running_state[i].load(std::memory_order_acquire) == Plush::Environment::RunningState::Running)
					{
						...
						Utilities::logline_threadsafe << i 
							<< std::setw(30) << effort 
