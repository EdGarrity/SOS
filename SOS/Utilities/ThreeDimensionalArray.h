...
namespace Utilities
{
	...
	public:
		...
		ThreeDimensionalArray(size_t x, size_t y) : data(x * y), x(x), y(y), z(1) {}
		...
		double& operator()(size_t i, size_t j) { return data[i + x * (j + y)]; }
		double operator()(size_t i, size_t j) const { return data[i + x * (j + y)]; }
	};
}
